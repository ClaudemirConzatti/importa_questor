...
$dtTarifa	= $_REQUEST['dtTarifa'];
    ...
    $sql_banco = "SELECT BancoCodigo,CodigoAgencia,CodigoCedente, DigitoCodigoCedente FROM $base.boleto LIMIT 1 ";
    $y_banco   = mysqli_query($con_mysql,$sql_banco);
    while($x_banco = mysqli_fetch_array($y_banco)){
        $conta = $x_banco['CodigoCedente'].$x_banco['DigitoCodigoCedente'];
        $conta = str_pad($conta , 8 , '0' , STR_PAD_LEFT);
		$Ag_Financeiro =  str_pad($x_banco['CodigoAgencia'], 4 , '0' , STR_PAD_LEFT);;
		$Conta_Financeiro =  str_pad($x_banco['CodigoCedente'], 7 , '0' , STR_PAD_LEFT);;
        $BANCO_FINANCEIRO = $x_banco['BancoCodigo'];
        $Banco_financeiro_confere = str_pad($x_banco['BancoCodigo'], 3 , '0' , STR_PAD_LEFT);
                ...
                <?php
                    ...
                    if (isset($_FILES)) {
                        ...
                        while(!feof($arquivo)){
                            ...
                            if($identificador==7 && $operacao=='RETORNO'){
                                $nota = substr($linha,38,24);
                                $nota = preg_replace("/[^0-9]/", "", $nota);
                                
                                $nosso_numero = intval(substr($linha,71,9));
...
/**********************************************************************************************************************/								
                                ...
                                $dt_credito = substr($linha,179,2).'-'.substr($linha,177,2).'-'.substr($linha,175,2);
...
/**********************************************************************************************************************/								
                                ...
                                $vlr_recebido = substr($linha,305,11).'.'.substr($linha,316,2);
